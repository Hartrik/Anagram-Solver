...
import cz.hartrik.anagram.solve.Anagram;
...
public class MainWindowController implements Initializable {
    ...
    protected String update() {
        ...
        final String normalized = Anagram.normalize(input);
    ...
    private void solve(final String anagram) {
        AnagramSolver solver = new AnagramSolver(anagram);

        if (cbDictCzech.isSelected())
            solver.search(cz.getPart(anagram));

        if (cbDictEnglish.isSelected())
            solver.search(en.getPart(anagram));

        if (cbDictUser.isSelected() && dictionaryManager != null)
            dictionaryManager.getController().userDictionaries()
                    .forEach(dict -> solver.search(dict.getPart(anagram)));

        Stream<String> stream = solver.getResults().stream();
        stream = cbSorted.isSelected() ? stream.sorted() : stream;
        final String results = stream.collect(Collectors.joining("\n"));

        Platform.runLater(() -> taResult.setText(results));
    }
